const PlayFabService = {
  ...
  getSession() {
    const user = localStorage.getItem("marrowgrow_session_user");
    const ticket = localStorage.getItem("marrowgrow_session_ticket");
    const timestamp = localStorage.getItem("marrowgrow_session_timestamp");

    if (user && ticket && timestamp) {
  ...
  autoLogin() {
    ...
    if (session.authMethod === "google") {
      // Google users need fresh login (but could implement token refresh)
      this.clearSession();
      return null;
    } else {
      // CustomID users can auto-login
      return PlayFab.ClientApi.LoginWithCustomID({
        CustomId: session.user,
        CreateAccount: false,
      });
    }
  },
